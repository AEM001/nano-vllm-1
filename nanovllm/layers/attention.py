...
def _causal_attention(q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, scale: float) -> torch.Tensor:
    q_t = q.transpose(0, 1)
    k_t = k.transpose(0, 1)
    v_t = v.transpose(0, 1)
    o = torch.nn.functional.scaled_dot_product_attention(q_t, k_t, v_t, is_causal=True, scale=scale)
...
def _decode_fallback(q: torch.Tensor, k_cache: torch.Tensor, v_cache: torch.Tensor, scale: float, num_heads: int) -> torch.Tensor:
    ...
    for i in range(num_seqs):
        ...
        k_i = _repeat_kv_heads(k_i, num_heads)
        v_i = _repeat_kv_heads(v_i, num_heads)
        outputs.append(_causal_attention(q[i:i + 1], k_i, v_i, scale))
    return torch.cat(outputs, dim=0)
